...
import { getYearlyIncomeValue, getYearlyStreamValue } from "../state/calculators";
...
function HourlyIncomeRow({ stream, payPeriodDays, adding, setAdding }: IncomeRowProps) {
  if (stream.kind !== "hourly") {
    throw new Error("HourlyIncomeRow can only be used with hourly income streams");
  }
  return (
    <tr key={stream.name} className="expense">
      <td></td>
      <td>{stream.name}</td>
      ...
      <td>${getYearlyStreamValue(stream, payPeriodDays).toFixed(2)}</td>
...
function SalaryIncomeRow({ stream, payPeriodDays, adding, setAdding }: IncomeRowProps) {
        ...
        <tbody className="body">
          ...
          {salaryStreams.map((stream) => (
            <SalaryIncomeRow
              key={stream.id}
              stream={stream}
              payPeriodDays={income.payPeriodDays}
              adding={adding}
              setAdding={setAdding}
            />
          ))}
          ...
          <tr className={adding || !hasStreams ? "add" : "hidden"}>
            ...
            <td>other</td>
            <td>
              <button onClick={handleAddStream} className={addReady ? "ready" : ""} disabled={!hasStreams && !addReady}>
          ...
          <tr className="total">
            ...
            <td></td>
            <td></td>
            <td>{`$${getYearlyIncomeValue(income).toFixed(2)}`}</td>
          </tr>
        </tbody>
      </table>
    </>
